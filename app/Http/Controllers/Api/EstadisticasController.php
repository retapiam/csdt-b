...
{
    /**
     * Obtener estadísticas para la página de inicio (públicas)
     */
    public function estadisticasInicio(Request $request)
    {
        try {
            // Contar casos procesados (combinamos varias fuentes)
            $casosProcesados = 0;
            
            // Intentar contar desde diferentes tablas
            try {
                $casosProcesados += PQRSFD::count();
            } catch (\Exception $e) {}
            
            try {
                $casosProcesados += Veeduria::count();
            } catch (\Exception $e) {}
            
            try {
                $casosProcesados += AnalisisIA::count();
            } catch (\Exception $e) {}
            ...
            try {
                ...
                try {
                    $usuariosActivos = Usuario::where('updated_at', '>=', now()->subDays(30))->count();
                } catch (\Exception $e2) {
                    $usuariosActivos = User::count();
                }
            }

            // Documentos generados (archivos + documentos de análisis)
            $documentosGenerados = 0;
            try {
                $documentosGenerados = Archivo::count();
            } catch (\Exception $e) {}
            
            // Agregar otros documentos generados
            try {
                $documentosGenerados += PQRSFD::whereNotNull('numero_radicacion')->count();
            } catch (\Exception $e) {}

            // Análisis completados
            $analisisCompletados = 0;
            try {
                $analisisCompletados = AnalisisIA::count();
            } catch (\Exception $e) {}
            
            try {
                $analisisCompletados += Veeduria::where('estado', 'completado')->count();
            } catch (\Exception $e) {}

            $estadisticas = [
                'casosProcesados' => $casosProcesados,
                'usuariosActivos' => $usuariosActivos,
                'documentosGenerados' => $documentosGenerados,
                'analisisCompletados' => $analisisCompletados,

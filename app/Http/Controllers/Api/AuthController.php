...
class AuthController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        try {
            ...
            $datos = [
                'nom' => $request->nom ?? $request->nombre ?? $request->nombres ?? '',
                'ape' => $request->ape ?? $request->apellido ?? $request->apellidos ?? '',
                'cor' => $request->cor ?? $request->email ?? $request->correo ?? '',
                'con' => $request->con ?? $request->password ?? $request->pass ?? $request->contrasena ?? '',
                'con_confirmation' => $request->con_confirmation ?? $request->password_confirmation ?? $request->pass_confirmation ?? $request->contrasena_confirmacion ?? '',
                'tel' => $request->tel ?? $request->telefono ?? '',
                'doc' => $request->doc ?? $request->numeroDocumento ?? $request->documento ?? '',
                'tip_doc' => $request->tip_doc ?? $request->tipoDocumento ?? $request->tipo_documento ?? 'cc',
                'rol' => $request->rol ?? 'cli',
                'est' => 'act', // Activo por defecto - sin restricciones de permisos
                'ciu' => $request->ciu ?? $request->ciudad ?? '',
                'dep' => $request->dep ?? $request->departamento ?? '',
                'dir' => $request->dir ?? $request->direccion ?? '',
                'gen' => $request->gen ?? $request->genero ?? 'n',
                'fec_nac' => $request->fec_nac ?? $request->fecha_nacimiento ?? null,
            ];
            ...
            return response()->json([
                ...
                'data' => [
                    'user' => $usuario,
                    'message' => 'Usuario registrado exitosamente. Puedes iniciar sesión inmediatamente.'
                ]
            ], 201);

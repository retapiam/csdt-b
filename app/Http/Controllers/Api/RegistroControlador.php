...
class RegistroControlador extends Controller
{
    ...
    private function validarCedulaUnica($documento_identidad)
    {
        ...
        $existeUsuario = Usuario::where('doc', $documento_identidad)->exists();
        if ($existeUsuario) {
    ...
    {
        // Verificar en usuarios
        $existeUsuario = Usuario::where('cor', $email)->exists();
        if ($existeUsuario) {
    ...
    public function registrar(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'rol_solicitado' => 'required|string|in:cliente,operador,administrador,administrador_general',
        ...
        try {
            // Crear usuario directamente sin restricciones de permisos
            $usuario = Usuario::create([
                'nom' => $request->nombre,
                'ape' => $request->apellidos ?? '',
                'cor' => $request->email,
                'con' => Hash::make('temporal123'), // Contraseña temporal
                'rol' => $this->mapearRol($request->rol_solicitado),
                'est' => 'act',
                'cor_ver' => true,
                'cor_ver_en' => now()
            ]);
            ...
            return response()->json([
                'success' => true,
                'message' => 'Registro exitoso. Puedes iniciar sesión con tus credenciales.',
                'data' => [
                    'id' => $usuario->id,
                    'email' => $usuario->cor,
                    'rol' => $usuario->rol,
    ...
    /**
     * Mapear rol solicitado a tipo de usuario
     */
    private function mapearRol(string $rolSolicitado): string
    {
        $mapeo = [
            'cliente' => 'cli',
            'operador' => 'ope',
        ...
        return $mapeo[$rolSolicitado] ?? 'cli';
    }
